#line 1 "/tmp/RNode_Firmware/Utilities.h"
...
uint8_t boot_vector = 0x00;
	...
	unsigned long get_rng_seed() {
		...
		for (int i = 0; i < 4; i++) {
			...
			rb[i] = nrf_rng_random_value_get(NRF_RNG);
			nrf_rng_event_clear(NRF_RNG, NRF_RNG_EVENT_VALRDY);
		}
		nrf_rng_task_trigger(NRF_RNG, NRF_RNG_TASK_STOP);
		unsigned long seed = (unsigned long)rb[0] << 24 | (unsigned long)rb[1] << 16 | (unsigned long)rb[2] << 8 | rb[3];
		seed ^= micros();
		return seed;
	}
#endif
  ...
  void boot_seq() { }
    ...
    void led_tx_off() { digitalWrite(pin_led_tx, HIGH); }
		...
		// XIAO nRF52840 user LEDs are active LOW (drive LOW = lit)
		void led_rx_on()  { digitalWrite(pin_led_rx, LOW); }
		void led_rx_off() {	digitalWrite(pin_led_rx, HIGH); }
		void led_tx_on()  { digitalWrite(pin_led_tx, LOW); }
		void led_tx_off() { digitalWrite(pin_led_tx, HIGH); }
...
void promisc_disable() {
  ...
  bool eeprom_begin() {
      ...
      if (!file) {
        ...
        uint8_t first_byte;
        void* fb_ptr = &first_byte;
        file.read(fb_ptr, 1);
        file.close();
        if (first_byte == 0xFF && file.open(EEPROM_FILE, FILE_O_WRITE)) {
          return true;
        }
        InternalFS.format();
        if (file.open(EEPROM_FILE, FILE_O_WRITE)) {
...
bool eeprom_product_valid() {
	...
	#elif PLATFORM == PLATFORM_NRF52
	if (rval == PRODUCT_RAK4631 || rval == PRODUCT_HELTEC_T114 || rval == PRODUCT_TECHO || rval == PRODUCT_HMBRW || rval == PRODUCT_T1000E || rval == PRODUCT_XIAO_NRF52) {
	#else
	if (false) {
	#endif
		return true;
