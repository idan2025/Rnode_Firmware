#line 1 "/tmp/RNode_Firmware/Boards.h"
...
#include "Modem.h"
  ...
  #define PRODUCT_T1000E      0x1E // Seeed SenseCAP Wio Tracker T1000-E
  #define BOARD_T1000E        0x52
  #define MODEL_B5            0xB5 // T1000-E, LR1110, 863-928 MHz

  #define PRODUCT_XIAO_NRF52  0x21 // Seeed XIAO nRF52840 + Wio-SX1262 for XIAO (SKU 102010710)
  #define BOARD_XIAO_NRF52    0x53
  #define MODEL_C0            0xC0 // XIAO nRF52840 + Wio-SX1262, 902-928 MHz, 22 dBm
  ...
  #define MODEL_FF            0xFF // Homebrew board, max 14dBm output power
    ...
    #define MCU_VARIANT MCU_NRF52
    #ifndef BOARD_MODEL
      #define BOARD_MODEL BOARD_GENERIC_NRF52
    #endif
  #else
      ...
      #define MODEM LR1110
    #elif BOARD_MODEL == BOARD_XIAO_NRF52
      #define MODEM SX1262
    #else
      #define MODEM SX1276
    #endif
  ...
  #define VALIDATE_FIRMWARE true
    ...
    #define PIN_GPS_RX 34
      ...
      // Pin mapping sourced from Seeed's official Arduino BSP
      // (Seeed-Studio/Adafruit_nRF52_Arduino, variant
      // Seeed_Tracker_T1000_E_LoRaWAN and the LBM_T1000E library)
      #define MODEM LR1110
      #define HAS_EEPROM false
      #define HAS_DISPLAY false
      #define HAS_BLUETOOTH false
      ...
      // LR1110 SPI and control pins. The RF switch and TCXO on this
      // board are controlled internally by the LR1110 via system
      // commands (set_dio_as_rf_switch / set_tcxo_mode), not by
      // dedicated MCU GPIOs, so no rxen/txen/tcxo_enable pins exist.
      const int pin_cs = 12;       // PIN_SPI_CS
      const int pin_sclk = 11;     // PIN_SPI_SCK
      const int pin_mosi = 41;     // PIN_SPI_MOSI
      const int pin_miso = 40;     // PIN_SPI_MISO
      const int pin_busy = 7;      // LR1110_BUSY_PIN
      const int pin_dio = 33;      // LR1110_IRQ_PIN
      const int pin_reset = 42;    // LR1110_NRESET_PIN
      const int pin_tcxo_enable = -1;

      // Battery / VBUS sense ADC pins (not yet exposed over KISS,
      // kept here for future use)
      const int pin_vbat = 2;      // A0, battery level divider
      const int pin_vcc = 4;       // A1, VCC voltage divider
      const int pin_vchg = 5;      // A2, charger insert detect

    #elif BOARD_MODEL == BOARD_XIAO_NRF52
      // Seeed XIAO nRF52840 + Wio-SX1262 for XIAO (kit SKU 102010710)
      // nRF52840 + Semtech SX1262. Wiring from Seeed's "Wio-SX1262 for XIAO"
      // schematic (V1.0), cross-checked against Meshtastic's validated
      // seeed_xiao_nrf52840_kit variant. XIAO D-labels -> nRF52840 GPIO:
      //   D1=GPIO1 D2=GPIO2 D3=GPIO3 D4=GPIO4 D5=GPIO5
      //   D8=GPIO8(SCK) D9=GPIO9(MISO) D10=GPIO10(MOSI)
      // RF switch: RX side on D5 (pin_rxen); TX side driven by SX1262 DIO2.
      // TCXO: on SX1262 DIO3 at 1.8V (see sx126x::enableTCXO()).
      #define MODEM SX1262
      #define HAS_EEPROM false
      #define HAS_DISPLAY false
      #define HAS_BLUETOOTH false
      // BLE OFF by default: Bluefruit.begin() wedges on this board (SD re-enable
      // hang), so the radio+USB path is shipped BLE-less. setup() calls
      // sd_softdevice_disable() before eeprom_begin so InternalFS uses the
      // direct-NVMC flash path (patched flash_nrf5x.c). USB-CDC serial is the
      // only transport.
      #define HAS_BLE false
      #define HAS_CONSOLE false
      #define HAS_PMU false
      ...
      #define EEPROM_OFFSET EEPROM_SIZE-EEPROM_RESERVED
      #define BLE_MANUFACTURER "Seeed"
      #define BLE_MODEL "XIAO-SX1262"

      const int pin_btn_usr1 = -1; // XIAO nRF52840 (non-Sense) has no user button

      // SX1262 control + SPI pins (XIAO nRF52840 GPIO numbers)
      const int pin_rxen = 5;      // D5  RF switch RX enable
      const int pin_txen = -1;     // TX side driven by SX1262 DIO2
      const int pin_reset = 2;     // D2  NRST
      const int pin_cs = 4;        // D4  NSS
      const int pin_sclk = 8;      // D8  SCK
      const int pin_mosi = 10;     // D10 MOSI
      const int pin_miso = 9;      // D9  MISO
      const int pin_busy = 3;      // D3  BUSY
      const int pin_dio = 1;       // D1  DIO1 (IRQ)
      const int pin_led_rx = LED_GREEN;  // GPIO13 (active low) - green blinks on RX
      const int pin_led_tx = LED_GREEN;  // GPIO13 (active low) - green blinks on TX
      const int pin_tcxo_enable = -1;    // TCXO powered via SX1262 DIO3
