...
5. **TCXO config corrected against Seeed's BSP (v1.4):** `enableTCXO()` originally called `lr11xx_system_set_tcxo_mode(CTX, LR11XX_SYSTEM_TCXO_CTRL_1_8V, 164)` — 1.8V supply, 164 ticks. A prior session changed this to `1_8V, 983` based on a misread of a different reference (it claimed Seeed's `smtc_modem_hal_get_radio_tcxo_startup_delay_ms()` = 30ms = 983 ticks). Pulling Seeed's actual LR1110 BSP source (`Seeed-Tracker-T1000-E-for-LoRaWAN-dev-board/t1000_e/LR11XX/smtc_shield_lr11xx/common/src/smtc_shield_lr11x0_common.c`) shows the canonical config is **`LR11XX_SYSTEM_TCXO_CTRL_3_0V, 164`** — the T1000-E's TCXO is a 3.0V part, not 1.8V. The previous 1.8V setting under-drove the TCXO, which can cause unreliable startup / wrong oscillation amplitude and silently mistune the carrier (TX_DONE still fires, frequency readback still looks right, but the RF is off-channel so peers see it weak or not at all). Fixed to `3_0V, 164` byte-for-byte per Seeed's BSP.

6. **RF switch `tx`/`tx_hp` mistuned — THE range-loss root cause (fixes #4's residual weakness):** the previous `rfswitch_cfg` in `lr1110::begin()` omitted `RFSW3` (DIO8) from both the `tx` and `tx_hp` switch positions, and set `wifi` to `RFSW3_HIGH` (which Meshtastic's T1000-E variant table has as all-low). The T1000-E's external RF switch uses DIO5/6/7/8 as four control lines; with DIO8 left low in TX/TX_HP, the switch never selects the correct TX path, so the LR1110's PA is biased to the requested dBm (e.g. 22 via the HP table from fix #4) but the RF is routed through a mistuned switch position and radiated power collapses far below the requested level. This is why a T1000-E running this firmware at TXP=22 is dramatically weaker than the same device running Meshtastic (which uses the correct table): fix #4 was necessary but not sufficient — the RF switch still threw away the gain. Fixed by porting Meshtastic's `variants/nrf52840/tracker-t1000-e/rfswitch.h` table byte-for-byte into the Semtech `lr11xx_system_rfswitch_cfg_t` bitfield form (also confirmed against Seeed's `smtc_shield_lr11x0_common.c::rfswitch_cfg`):
...
7. **PA bias table wrong at 20/21/22 dBm (v1.4):** fix #4's per-dBm PA tables were verified byte-for-byte against Seeed's BSP for the LP range (-17..15) and HP 16..19, but the HP entries at **20, 21, 22 dBm** were wrong. At TXP=22 (the typical max setting): firmware had `duty_cycle=0x04, hp_sel=0x07`, Seeed's canonical `pa_cfg_table[]` says `duty_cycle=0x06, hp_sel=0x07`. The `pa_duty_cycle` field controls the PA duty cycle (0.2 + 0.04×duty); firmware's 0x04 = 0.36, Seeed's 0x06 = 0.44 — the firmware was **under-driving the HP PA by ~22% of the duty cycle at 22 dBm**, a real measurable power loss stacked on top of the RF-switch bug (#6). At 21 dBm the `hp_sel` was also wrong (0x06 vs 0x07). Fixed by regenerating the entire PA table (LP + HP) byte-for-byte from Seeed's `LR1110MB1DxS/smtc_shield_lr1110mb1dxs.c::pa_cfg_table[]` via a Python compare script that verifies all 40 entries (LP -17..15 + HP 16..22). Also removed the bogus fabricated HP entries that used to cover -9..15 (Seeed never defines HP for <16; the LP/HP branch picks LP there anyway, so they were unreachable but misleading).
