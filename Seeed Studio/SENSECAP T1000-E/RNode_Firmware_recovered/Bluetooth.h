#line 1 "/tmp/RNode_Firmware/Bluetooth.h"
...
char bt_devname[11];
  ...
  // Bonding only authenticates the *pairing* handshake, which the
  // firmware immediately disconnects after (see bt_pairing_complete).
  // A later plain reconnect to an already-bonded central is not
  // automatically re-encrypted by every host BLE stack, and the UART
  // characteristics require an encrypted link (SECMODE_ENC_WITH_MITM).
  // Without explicitly requesting it here, such a reconnect can stay
  // GATT-connected but unsecured forever: writes are silently dropped,
  // bt_connect_callback (bound to the Secured event) never fires, and
  // bt_state never reaches BT_STATE_CONNECTED, so the firmware never
  // reads the link - looking exactly like "pairs but doesn't connect".
  // requestPairing() is a no-op if already secured, and silently
  // reuses the stored bond/LTK instead of prompting for a new PIN.
  void bt_periph_connect_callback(uint16_t conn_handle) {
    // Serial.println("Periph connect callback");
    BLEConnection* conn = Bluefruit.Connection(conn_handle);
    conn->requestPairing();
  }
  ...
  void bt_disconnect_callback(uint16_t conn_handle, uint8_t reason) {
    ...
    // Without this, the nRF52 SoftDevice does not reliably resume
    // advertisements after a bonded disconnect, which breaks
    // reconnection from apps like Sideband / Columba.
  ...
  bool bt_setup_hw() {
    ...
    if (!bt_ready) {
      ...
      if (Bluefruit.begin()) {
        ...
        Bluefruit.Periph.setConnectCallback(bt_periph_connect_callback);
