...
3. **`release.json` asset on every release** (since v1.7): `{"rnode_firmware_t1000e.zip": {"version": "<tag name>", "hash": "<sha256 of the zip>"}}`. `tools/rnodeconf_t1000e.py -u` fetches `releases/latest/download/release.json`, then `releases/download/<version>/rnode_firmware_t1000e.zip`, so `version` must equal the tag and `hash` must match the zip, or rnodeconf refuses to flash. Clobbering the zip on an existing release means regenerating release.json too.
